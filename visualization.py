...
class EvaluationCardsApp:
    def __init__(self, path, server=None):
        ...
        self.state.symbols_expanded = {
            c["id"]: {r["id"]: True} for c in self.state.cards for r in c["runs"]
        }
                    ...
                    for card_run in algorithm_dir.iterdir():
                        ...
                        if (card_run / "card.yaml").exists():
                            with open((card_run / "card.yaml"), "r") as f:
                                ...
                                card["category"] = (
                                    "Model Generalization"  # FIXME: hardcoded
                                )
                        else:
    ...
    def filter_cards(self, cards, search_term, category, result):
        ...
        if search_term:
            ...
            filtered = [
                ...
                or search_lower in c["card"]["description"].lower()
            ]
    ...
    def on_filter_change(self, search_term, category_filter, result_filter, **kwargs):
        self.state.filtered_cards = self.filter_cards(
            self.state.cards, search_term, category_filter, result_filter
        )
        ...
        with v3.VCard(variant="outlined"):
            ...
            with v3.VCardTitle(classes="pa-6 bg-grey-lighten-4"):
                with html.Div(classes="d-flex justify-space-between align-start"):
                    with html.Div(classes="flex-grow-1"):
                        html.H2(
                            "{{ selected_card.card.title }}", classes="text-h4 mb-2"
                        )
                        html.P(
                            "{{ selected_card.card.description }}",
                            classes="text-body-2 text-grey-darken-2 text-wrap",
                        )
                        ...
                        # )
                    with html.Div(
                        classes="d-flex align-start ms-4", style="white-space: nowrap;"
                    ):
                        with v3.VChip(
                            v_bind_color=(
            ...
            with v3.VCardText(classes="pa-6"):
                with v3.VBtn(
                    ...
                    html.H3("Runs ({{ selected_card.runs.length }})", classes="text-h6")
                ...
                with html.Div(v_if="runs_expanded"):
                    with html.Template(
                        ...
                        with v3.VCard(
                            ...
                            with html.Div(
                                classes="d-flex justify-space-between align-start mb-2"
                            ):
                            ...
                            with html.Div(
                                v_if="run.status !== 'VERIFIED'", classes="mb-3"
                            ):
                                with html.Div():
                                    with v3.VAlert(
                            ...
                            with html.Div(v_if="symbols_expanded"):
                                with html.Template(
                                    ...
                                    with v3.VCard(
                                        ...
                                        with html.Div(
                                            classes="d-flex justify-space-between mb-2"
                                        ):
                                            html.Code(
                                                "{{ name }}",
                                        ...
                                        with html.Div(
                                            ...
                                            html.Span(
                                                "Value: ",
                                                classes="font-weight-bold text-grey-darken-2",
                                            )
                                            html.Span(
                                                "{{ values }}",
                                                classes="font-family-monospace",
                                            )
